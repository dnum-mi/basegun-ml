...
class YOLOv5_OBB:
    ...
    def postprecess(self, prediction, src_img, new_shape):
        ...
        CONF_THRES =maxconf-0.1  # To retrieve best results and not limit to an absolute threshold
...
def load_models(model_card_path,model_weapon_path):
    ...
    model_weapon = YOLO('yolov8n-pose.pt') #necessary to load the base model before the pretrained weights
    model_weapon = YOLO(model_weapon_path)

    #warmup
    imagetest=cv2.imread("warmup.jpg")
...
def get_lengths(image,model_card,model_weapon,draw=True,output_filename="result.jpg"):
    """Predict the keypoints on the image
    Args:
        image (opencv matrix): image after CV2.imread(path)
        modelCard (model): model after load_models call
    ...
    factor=CardR/CardP
    canonP=distanceCalculate((int(keypoints[2][0]),int(keypoints[2][1])),(int(keypoints[3][0]),int(keypoints[3][1])))
    ...
    if draw:
        ...
        for keypoint in keypoints:
            img2 = cv2.circle(img2, (int(keypoint[0]),int(keypoint[1])) , radius=5, color=(0, 0, 255), thickness=20)
        
        img2 = cv2.line(img2, (int(card[0]),int(card[1])),(int(card[2]),int(card[3])) , color=(255, 0, 0), thickness=15)
        img2 = cv2.line(img2, (int(card[4]),int(card[5])),(int(card[2]),int(card[3])) , color=(255, 0, 0), thickness=15)
        img2 = cv2.line(img2, (int(card[6]),int(card[7])),(int(card[4]),int(card[5])) , color=(255, 0, 0), thickness=15)
        img2 = cv2.line(img2, (int(card[0]),int(card[1])),(int(card[6]),int(card[7])) , color=(255, 0, 0), thickness=15)
        img2 = cv2.line(img2, (int(card[0]),int(card[1])),(int(card[4]),int(card[5])) , color=(255, 255, 0), thickness=15)

...
from prepare_data import ConvertRgb, RandomPad, Rescale
...
def show_images_of_label_predicted_as(
    in_df: pd.DataFrame, true_label: str, pred_label: str, limit=20
):
    ...
    for index, row in df.head(limit).iterrows():
        ...
        im = Image.open(path)
        plt.subplot(len(df) / columns + 1, columns, index + 1).set_title(
            round(row[pred_label], 3)
        )
        plt.axis("off")
...
def convert_to_onnx(state_dict_path: str):
    ...
    np.testing.assert_allclose(
        pth_out.detach().cpu().numpy(), ort_outs[0], rtol=1e-03, atol=1e-05
    )
...
def roc_from_probas_file(csvpath: str):
    ...
    for i in range(len(CLASSES)):
        ...
        plt.plot(
            ...
            linestyle="--",
            color=COLORS[i],
            label=f"Class {CLASSES[i]} vs Rest",
        )

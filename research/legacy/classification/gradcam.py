...
trained_model.load_state_dict(
    torch.load("models/B5_2022-02-09_13/B5_2022-02-09_13.pth", map_location=device)[
        "model_state_dict"
    ]
)
...
def gradcam_heatmap(in_tensor: torch.Tensor, pred_index: int) -> np.array:
    ...
    print(
        f"GradCam for class {CLASSES[pred_index]}, ranked {1+list(torch.argsort(preds, dim=1, descending=True)[0]).index(pred_index)}/{len(CLASSES)}"
    )

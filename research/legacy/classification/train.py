...
from prepare_data import get_classes, get_dataloader, get_size, load_dataset
...
def train_model(
    model: Model, criterion, optimizer, scheduler, checkpoint: str = None
) -> Model:
...
def test_model(model: Model, training_name: str) -> None:
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(dataloaders["val"]):
            ...
            outputlist = torch.cat(
                [outputlist, torch.nn.functional.softmax(outputs, dim=1).cpu().detach()]
            )
    ...
    plt.savefig(
        f"models/{training_name}/confusion-matrix.jpg", dpi=150, bbox_inches="tight"
    )
    plt.close()
    ...
    all_lines = {
        "filename": [x[0] for x in dataset["val"].imgs],
...
if __name__ == "__main__":
    ...
    model, training_name = train_model(
        model, criterion, optimizer, lr_scheduler, checkpoint=None
    )
    # evaluate
    ...
    with open(f"models/{training_name}/details.txt", "a") as outfile:
        ...
        outfile.write(
            f'Optimizer = {optimizer.__class__.__name__} lr={round(optimizer.param_groups[0]["lr"], 4)}\n'
        )
        outfile.write(
            f'Lr = {lr_scheduler.__class__.__name__} max_lr={round(optimizer.param_groups[0]["max_lr"], 4)}\n'
        )
